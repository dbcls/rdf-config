...
CSVファイル、TSVファイル、DuckDB、SQLite3からRDFやJSON-LDを生成するルール（処理手順）を定義するファイルで、下記のYAML形式で記述する。
...
1. 目的語名1-2の生成処理1が実行される。<br/>通常、最初の生成処理は`col(column_name)`で、カラムの値を取得する処理となる。
...
| col          | `col(col_name)`<br/>CSV, TSV, DuckDB, SQLite3のcol_nameカラムの値を取得する。|
...
| source       | `source(file_path[, file_format[, table_name]])`<br/>変換対象とするファイルのパス、ファイルのフォーマット、テーブル名（DuckDBの場合）を指定する。<br/>ファイルのフォーマットは、`:csv`, `:tsv`, `:duckdb`, `sqlite3` のいずれかを指定し、`:csv`, `:tsv`の場合は省略可能である。（この場合、入力ファイルの拡張子からフォーマットを決定する）<br/>`:duckdb`, `:sqlite3`を指定した場合は、table_nameも指定する必要がある。|
