    ...
    for item in items {
        match item {
            Value::String(s) => {
                ...
                if let Some((path, fmt, table)) = parse_source_call(s)
                    .with_context(|| format!("In subject rule '{}': invalid source() call", name))?
                {
...
/// Parse `source("path"[, :format[, "table"]])` from a string.
///
/// Returns `Ok(None)` if `s` is not a `source(...)` call,
/// `Ok(Some((path, format, table)))` if valid, and `Err(...)` if malformed.
///
/// Validation:
/// - 1 arg: path only (format inferred later from extension)
/// - 2 args: path + format. `:duckdb` here without a 3rd arg is an error.
/// - 3 args: path + format + table. Format must be `:duckdb`.
fn parse_source_call(s: &str) -> Result<Option<(String, Option<SourceFormat>, Option<String>)>> {
    let s = s.trim();
    ...
    if args.is_empty() {
        bail!("source() requires at least one argument (file path)");
    }
    if args.len() > 3 {
        bail!("source() accepts at most 3 arguments, got {}", args.len());
    }

    let path = args[0].clone();
    let format = match args.get(1) {
        Some(raw) => Some(parse_format_keyword(raw)?),
        None => None,
    };
    let table = args.get(2).cloned();

    // Cross-argument validation
    match (format, table.as_deref()) {
        (Some(fmt), None) if is_database_format(fmt) => {
            bail!(
                "source() with :{} requires a third argument (table name): \
                 source(\"{}\", :{}, \"<table>\")",
                format_keyword_str(fmt), path, format_keyword_str(fmt)
            );
        }
        (Some(fmt), Some(_)) if !is_database_format(fmt) => {
            bail!(
                "source() third argument (table name) is only valid with database \
                 formats (:duckdb, :sqlite), not with :{}",
                format_keyword_str(fmt)
            );
        }
        (None, Some(_)) => {
            bail!(
                "source() third argument requires :duckdb or :sqlite as the second argument"
            );
        }
        _ => {}
    }

    Ok(Some((path, format, table)))
}

/// Parse a format keyword like `:duckdb`, `:sqlite`, `:tsv`, `:csv` (leading colon
/// optional, case-insensitive). `:sqlite3` is accepted as an alias for `:sqlite`.
fn parse_format_keyword(raw: &str) -> Result<SourceFormat> {
    let normalized = raw.trim().trim_start_matches(':').to_ascii_lowercase();
    match normalized.as_str() {
        "tsv" => Ok(SourceFormat::Tsv),
        "csv" => Ok(SourceFormat::Csv),
        "duckdb" => Ok(SourceFormat::DuckDb),
        "sqlite" | "sqlite3" => Ok(SourceFormat::Sqlite),
        _ => bail!(
            "Unknown source format: '{}' (expected :tsv, :csv, :duckdb, :sqlite, or :sqlite3)",
            raw
        ),
    }
}

fn format_keyword_str(fmt: SourceFormat) -> &'static str {
    match fmt {
...
/// Returns true for formats that require a table name (3rd arg of source()).
fn is_database_format(fmt: SourceFormat) -> bool {
    matches!(fmt, SourceFormat::DuckDb | SourceFormat::Sqlite)
}
