...
struct Cli {
    ...
    /// {...}}` document). The two JSON formats differ in default nesting:
    /// `:jsonld` is flat by default, `:jsonl` is nested by default. Override
    /// with `--nest` / `--flat`.
    #[arg(long, default_value = ":turtle")]
    convert: String,

    /// Force nested JSON output: inline referenced subjects inside their
    /// parent.
    ///
    /// A subject reached through a reference-valued predicate (model
    /// `Reference` / `ReferenceList`) is inlined inside its parent, a repeated
    /// reference becomes a JSON array of inlined objects, and referenced
    /// subjects are not emitted on their own. This is already the default for
    /// `:jsonl`; pass `--nest` to also nest `:jsonld`. Only valid with
    /// `--convert :jsonl` or `:jsonld`, and mutually exclusive with `--flat`.
    #[arg(long, conflicts_with = "flat")]
    nest: bool,

    /// Force flat JSON output: write references as bare IRI strings and emit
    /// every subject as its own node.
    ///
    /// For `:jsonl` this also explodes a repeated property so each repeated
    /// child becomes its own line. This is already the default for `:jsonld`;
    /// pass `--flat` to flatten `:jsonl`. Only valid with `--convert :jsonl`
    /// or `:jsonld`.
    #[arg(long)]
    flat: bool,
...
enum ConvertMode {
    ...
    NTriples,
    /// JSON-LD as one `{"@context": {...}, "@graph": [...]}` document with
    /// references written flat as bare IRIs. Default for `:jsonld`.
    JsonLd,
    /// JSON-LD as JSON Lines, references flat as bare IRIs and repeated
    /// properties exploded across lines (`:jsonl --flat`).
    JsonLines,
    /// The `@context`/`@graph` document with referenced subjects embedded
    /// inline (`:jsonld --nest`). Same wrapper as `JsonLd`.
    JsonLdNested,
    /// JSON Lines with referenced subjects embedded inline, one root subject
    /// per line and no line-explosion. Default for `:jsonl`.
    JsonLinesNested,
...
fn main() -> Result<()> {
    ...
    // Decide JSON nesting. The two JSON formats have different defaults
    // because they're consumed differently: `:jsonld` defaults to a flat
    // `@graph`, `:jsonl` defaults to nested documents. `--nest` / `--flat`
    // override the default; clap enforces they aren't both set. Either flag is
    // meaningless for the non-JSON formats and is rejected there.
    let out_mode = match out_mode {
        // `:jsonl` nests by default; `--flat` opts out.
        ConvertMode::JsonLines => {
            if cli.flat { ConvertMode::JsonLines } else { ConvertMode::JsonLinesNested }
        }
        // `:jsonld` is flat by default; `--nest` opts in.
        ConvertMode::JsonLd => {
            if cli.nest { ConvertMode::JsonLdNested } else { ConvertMode::JsonLd }
        }
        other => {
            if cli.nest || cli.flat {
                bail!("--nest / --flat are only valid with --convert :jsonl or :jsonld");
            }
            other
        }
    };
    ...
    match out_mode {
        ...
        // The embedded `@graph` document uses the same prelude.
        ConvertMode::JsonLdNested => jsonld::write_prelude(&mut buf_writer, &prefixes, &name_maps)?,
        // JSON Lines has no document wrapper — nodes start straight away.
        ConvertMode::JsonLines => {}
        // Embedded JSON Lines likewise has no wrapper.
        ConvertMode::JsonLinesNested => {}
    ...
    if matches!(out_mode, ConvertMode::JsonLd | ConvertMode::JsonLdNested) {
            ...
            #[cfg(not(feature = "duckdb"))]
            {
                bail!(
                    "DuckDB input ({}#{}) requires a build with DuckDB support. \
                     ...
                     (or `--features all` for both DuckDB and SQLite).",
                    path.display(), table
                )
            }
        }
...
fn flush_chunk<W: Write>(
    ...
    // Nested output embeds each referenced child into its parent, so a child
    // shared across rows (e.g. a gene referenced by every variant) must be
    // present in *every* chunk that references it. The global `seen` set would
    // drop that child's defining triples after the first chunk it appears in,
    // leaving later chunks with only the bare-IRI edge and no node to inline —
    // the same reference would render as a full object in early documents and
    // a bare string afterwards. So for the nested modes we deduplicate within
    // the chunk only (a throwaway set): identical triples from sibling rows are
    // still collapsed, so an embedded node is never doubled, but each chunk
    // keeps its own copy of the shared child and embedding stays consistent.
    // Every referenced child is generated from the same input row as its
    // parent, so it always lands in the same chunk — no cross-chunk state is
    // needed. The flat modes keep the cross-chunk `seen` dedup unchanged.
    let unique = match out_mode {
        ConvertMode::JsonLdNested | ConvertMode::JsonLinesNested => {
            let mut chunk_seen: HashSet<u64> = HashSet::new();
            dedup_triples(triples, &mut chunk_seen, usize::MAX)
        }
        _ => dedup_triples(triples, seen, dedup_limit),
    };
    ...
    match out_mode {
        ...
        ConvertMode::JsonLdNested => {
            jsonld::write_triples_nested(
                writer, &validated, prefixes, name_maps, first_subject,
                &convert_subjects, jsonld::JsonLdMode::Graph,
            )?;
        }
