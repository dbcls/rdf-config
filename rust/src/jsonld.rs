...
//!
//! Output modes (selected by `--convert`, nesting overridable with
//! `--nest` / `--flat`):
//!   * `:jsonld` — the single enclosing document shown above. Flat by
//!     default: every subject is its own `@graph` node with references as
//!     bare IRIs (`write_triples(JsonLdMode::Graph)`). `--nest` instead nests
//!     referenced subjects (`write_triples_nested(JsonLdMode::Graph)`). Both
//!     share `write_prelude` / `write_postlude`.
//!   * `:jsonl` — JSON Lines for Elasticsearch: one document per line, each
//!     carrying `"@context": "context.jsonld"` instead of the inline context.
//!     Nested by default, one root subject per line
//!     (`write_triples_nested(JsonLdMode::Lines)`). `--flat` instead writes
//!     references as bare IRIs and explodes a subject with a repeated
//!     (array-valued) property so each repeated child becomes its own document
//!     (`write_triples(JsonLdMode::Lines)`). No prelude/postlude either way.
//!   * `:context` — just the `{ "@context": { ... } }` document, written by
//!     `write_context_document`, suitable for saving as `context.jsonld`.
//!
//! Nesting inlines a referenced subject inside its referencing parent instead
//! of emitting it as a separate node; see `write_triples_nested`. In the
//! nested `:jsonl` form the flat line-explosion is replaced by one line per
//! root subject, with children — including repeated ones, as JSON arrays —
//! nested in place. The two JSON formats default differently (`:jsonld` flat,
//! `:jsonl` nested) because they're consumed differently.
...
/// Write a chunk as *nested* (framed) JSON-LD: a subject reached through a
/// reference-valued predicate is inlined inside its referencing parent in
/// place of the bare IRI string, and a referenced subject is no longer
/// emitted as a top-level node of its own.
///
/// This is the embedded counterpart to [`write_triples`]. References that get
/// inlined are exactly those that serialize as a bare IRI string under an
/// `@type: @id` binding — i.e. the model's `Reference` / `ReferenceList`
/// objects (and any `Uri` object whose value happens to name another emitted
/// subject). A plain `Uri` value whose target is *not* an emitted subject
/// (e.g. an external assembly IRI) is left as a string, exactly as in the flat
/// output.
///
/// Mode handling:
///   * `JsonLdMode::Lines` — each *root* subject (one not referenced by any
///     other subject in the chunk) becomes a single self-contained line, with
///     its children inlined. Unlike the flat Lines path there is **no**
///     `explode_node` step: a repeated reference stays a JSON array of inlined
///     objects rather than being split across lines.
///   * `JsonLdMode::Graph` — only root subjects are written into `@graph`;
///     each carries its embedded children.
///
/// Scope is a single chunk: a parent and the subjects it references must be
/// processed together for the children to be inlined. The engine emits every
/// triple generated from one input row together, so a subject and the child
/// subjects it points at land in the same chunk whenever they come from the
/// same row — the usual case for denormalized tabular data. A reference whose
/// target is not a subject in the current chunk is left as a bare IRI string.
///
/// Because embedding is chunk-local, the caller must ensure a shared child
/// (one referenced from many rows) is present in *every* chunk that references
/// it rather than emitted once and then deduplicated away. `flush_chunk` does
/// this by deduplicating the nested modes within each chunk only, so a child
/// generated by sibling rows survives once per chunk. Cross-chunk
/// deduplication (the flat modes' global `seen` set) would instead leave later
/// chunks with a bare IRI and no node to inline.
///
/// Node contents — scalars, plain `Uri`-valued properties, key order within
/// each node and number formatting — are produced by the same
/// [`build_named_node`] path as the flat writer, so an inlined child is byte
/// for byte the node the flat writer would have emitted on its own line,
/// minus its line wrapper.
pub fn write_triples_nested<W: Write>(
    writer: &mut W,
    triples: &[Triple],
    prefixes: &PrefixMap,
    names: &NameMaps,
    ...
    // Drop prefixes shadowed by an object binding, matching `write_triples`,
    // so reference strings and node `@id`s compact through the same prefix
    // table and the string-keyed embedding below lines up exactly.
    let prefixes = &effective_prefixes(prefixes, names);
    let grouped = group_triples(triples);
    ...
    // Build every candidate top-level subject's node up front: a parent needs
    // its children's nodes available before it can inline them. `node_by_cid`
    // is keyed by the *compact IRI* — the exact string form a reference to
    // this subject takes as a property value — so embedding is a plain
    // string-keyed lookup. Insertion order is preserved for stable output.
    let mut node_by_cid: IndexMap<String, Value> = IndexMap::new();
    // Compact IRIs referenced by some other subject in this chunk; these
    // become embedded-only and are not emitted at the top level.
    ...
    for (subject, items) in &grouped {
        ...
        let cid = compact_iri(subject, prefixes);

        // Record outgoing references to other subjects. We compact the target
        // to the same form used as the node key so the membership test (run
        // after every candidate is known) is exact. A self-reference is never
        // treated as an embedding edge.
        for it in &filtered {
            let target = match &it.object {
                RdfTerm::Uri(u) => Some(compact_iri(u, prefixes)),
                RdfTerm::Curie(c) => Some(compact_iri(c, prefixes)),
                _ => None,
            };
        ...
        let node = build_named_node(subject, &filtered, &grouped, prefixes, names, &type_only);
        node_by_cid.insert(cid, node);
    }

    // A reference only embeds if its target is actually a subject we built in
    // this chunk; intersect against the known node set.
    referenced.retain(|cid| node_by_cid.contains_key(cid));

    // Roots are subjects not referenced by anyone. If a reference cycle leaves
    // no roots at all (impossible for the tree-shaped data tabular conversion
    // produces, but guarded so nothing silently vanishes), fall back to
    // emitting every candidate as a root; the `visited` cycle guard in
    // `embed_into` still terminates and keeps each line finite.
    let no_roots = !node_by_cid.is_empty()
        && node_by_cid.keys().all(|cid| referenced.contains(cid));

    for (cid, node) in &node_by_cid {
        if !no_roots && referenced.contains(cid) {
            continue; // embedded into its parent instead of standing alone
        }

        let mut visited: HashSet<String> = HashSet::new();
        visited.insert(cid.clone());
        let embedded = embed_into(node.clone(), &node_by_cid, Some(cid.as_str()), &mut visited);
        ...
        match mode {
            JsonLdMode::Graph => {
                ...
                write_indented_value(writer, &embedded, 4)?;
            }
            JsonLdMode::Lines => {
                let doc = prepend_context_ref(embedded);
                let line = serde_json::to_string(&doc)
                    .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
                writeln!(writer, "{}", line)?;
            }
...
/// Recursively inline referenced subjects into a built node.
///
/// A JSON string value is treated as an embedding edge when it names a subject
/// in `node_by_cid` (the compact-IRI key set) — except the node's own `@id`
/// value, which equals `self_id` and is left alone. When a string is an
/// embedding edge, it is replaced by a deep-embedded clone of the referenced
/// node; `self_id` is updated to that child's id for the recursion so the
/// child's own `@id` field isn't itself mistaken for a reference.
///
/// `visited` holds the ids currently being expanded on the path from the root
/// down to here. A reference back to one of them is a cycle that cannot be
/// written as finite nested JSON, so the bare IRI string is kept instead (the
/// other end of the cycle is still reachable as its own root), guaranteeing
/// termination.
///
/// Only bare-string references are inlined. A `{"id": "..."}` object — the
/// non-coerced URI form, which `Reference` / `ReferenceList` objects never
/// take — is walked into like any other object but not itself collapsed into
/// an embed, so it stays a plain link.
fn embed_into(
    value: Value,
    node_by_cid: &IndexMap<String, Value>,
    self_id: Option<&str>,
    visited: &mut HashSet<String>,
) -> Value {
    match value {
        Value::String(s) => {
            if Some(s.as_str()) != self_id
                && !visited.contains(&s)
                && node_by_cid.contains_key(&s)
            {
                let child = node_by_cid.get(&s).cloned().unwrap();
                visited.insert(s.clone());
                let embedded = embed_into(child, node_by_cid, Some(s.as_str()), visited);
                visited.remove(&s);
                embedded
            } else {
                Value::String(s)
        ...
        Value::Array(items) => Value::Array(
            items.into_iter()
                .map(|v| embed_into(v, node_by_cid, self_id, visited))
                .collect(),
        ),
        Value::Object(map) => {
            let mut m = Map::new();
            for (k, v) in map {
                m.insert(k, embed_into(v, node_by_cid, self_id, visited));
            }
            Value::Object(m)
        }
        other => other,
